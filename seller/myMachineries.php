...
<?php include '../connection.php' ?>
  <div class="d-flex" id="wrapper">
    ...
    <div id="page-content-wrapper">
      ...
      <div class="container-fluid">
        <h1 class="mt-4">My Machineries</h1>
  ...
  <tbody>
    ...
    include '../connection.php'; 
      ...
      $myCropQuery= "SELECT * FROM product WHERE phoneNumber='$phoneNumber' AND productType='machinery'";
      ...
      if(mysqli_num_rows($myCropQueryResult)>0){
        while($data= mysqli_fetch_assoc($myCropQueryResult)){
          ...
          echo "<tr><td class='machineryId'>".$data['productId']."</td><td>".$data['productName']."</td><td>".$data['productDescription']."</td><td><img src=$url width=100 height=100></td><td>".$data['productQuantity']." Pieces</td><td>".$data['productPrice']." RS/Piece</td><td><button class='btn btn-info m-1 addQuantity'>ADD</button><button class='btn btn-danger m-1 deleteMachinery'>Remove</button><button class='btn btn-success m-1 updatePrice'>Update</button></td></tr>";
        }
      }
    ?>
  </tbody>
  ...
  $(".addQuantity").click(function(){
   var machineryId= $(this).closest('tr').find('.machineryId').text();
   window.location.href="action.php?machineryId="+machineryId+"&action=addMachinery";
  });

  $(".deleteMachinery").click(function(){
    var machineryId= $(this).closest('tr').find('.machineryId').text();
    var confirmation= confirm('Are you sure you want to remove this Machinery?');
    if(confirmation==true){
    window.location.href="action.php?machineryId="+machineryId+"&action=deleteMachinery";
    }
  });

  $(".updatePrice").click(function(){
    var machineryId= $(this).closest('tr').find('.machineryId').text();
    window.location.href="action.php?machineryId="+machineryId+"&action=updatePrice";
  });
  </script>
