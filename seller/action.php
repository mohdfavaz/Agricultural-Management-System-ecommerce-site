    ...
    <div id="page-content-wrapper">
      ...
      <div class="container-fluid">
       ...
       include '../connection.php';
        $machineryId= $_GET['machineryId'];
        $action= $_GET['action'];
        switch($action){
            case 'addMachinery':
                echo "<form action='' method='POST'><div class='form-group'><label for='quantity'>Quantity</label><input type='number' class='form-control' name='quantity' aria-describedby='quantityHelp'><small id='quantityHelp' class='form-text text-muted'>New Quantity You want to add only needed to be mentioned!</small></div><button type='submit' class='btn btn-primary' name='addMachinery'>Submit</button></form>";
                break;
            case 'deleteCrop':
                $deleteCropQuery= "DELETE FROM product WHERE productId='$machineryId'";
                $deleteQueryResult= mysqli_query($conn,$deleteCropQuery);
                if($deleteQueryResult){
                  echo "<script>alert('Machinery Deleted Successfully');</script>";
        ...
        if(isset($_POST['addMachinery'])){
          $machineryId= $_GET['machineryId'];
          $newQuantity= $_POST['quantity'];
          $updateMachineryQuery= "UPDATE product SET productQuantity=productQuantity+$newQuantity WHERE productId='$machineryId'";
          $updateMachineryQueryResult= mysqli_query($conn,$updateMachineryQuery);
          if($updateMachineryQueryResult){
            echo "<script>alert('Machinery Quantity updated successfully');</script>";
            echo "<script>window.location.href='myMachineries.php';</script>";
          }
          else{
            echo "<script>alert('Something went wrong');</script>";
            echo "<script>window.location.href='myMachineries.php';</script>";
        ...
        if(isset($_POST['updatePrice'])){
          ...
          $updatePriceQuery= "UPDATE product SET productPrice='$newPrice' WHERE productId='$machineryId'";
          $updatePriceQueryResult= mysqli_query($conn,$updatePriceQuery);
          if($updatePriceQueryResult){
            echo "<script>alert('Machinery Price updated successfully');</script>";
            echo "<script>window.location.href='myMachineries.php';</script>";
